...
#$ENV{HTTPS_CA_FILE} = "ironportCA.pem";
#$ENV{HTTPS_CA_FILE} = "rui.crt";
...
} elsif (defined($datacenter)) {
        $url = 'https://$datacenter/sdk/vimService';
}
...
{
        if (-e $sessionfile && -s $sessionfile) 
        {
        ...
        Opts::set_option("savesessionfile", $sessionfile);
}
...
eval
{
	...
	{
                # Util::connect handles session loading
	...
	else
	{
        ...
        # Util::disconnect handles session saving
	#if (defined($sessionfile))
	#{
...
sub host_runtime_info
{
	...
	if (defined($subcommand))
	{
		...
		elsif (uc($subcommand) eq "ISSUES")
		{
			...
			if (defined($issues))
			{
				...
				if ((@$issues[$i]->fullFormattedMessage =~ m/tech support mode.*has been enabled/i) || (@$issues[$i]->fullFormattedMessage =~ m/syslog not configured/i)) {
				    $n_ignore++;
				}
			    }
			    if ($n_ignore == scalar(@$issues)) {
				$res = 'OK';
				$output = 'Ignored ' . $n_ignore . ' issue(s): Tech Support Mode (SSH) enabled and/or Syslog is not configured. No other config issues.';
			    }
			    else {
				$output = '';
				foreach (@$issues)
				{
                                        if (defined $_->vm) {
                                                $output .= $_->vm->name . ": " . $_->fullFormattedMessage . 
                                                        "(caused by " . $_->userName . "); ";
                                        } else {
                                                $output .= $_->fullFormattedMessage;
                                        }
				}
			    }
			}
	...
	else
	{
		...
		if (defined($cpuStatusInfo))
		{
			...
			{
				$SensorCount++;
				$AlertCount++ if ($_->status->key ne 'Green');
			}
		}
		...
		if (defined($numericSensorInfo))
		{
			...
			{
				$SensorCount++;
				$AlertCount++ if ($_->healthState->key ne 'Green');
			}
		}
...
sub dc_list_vm_volumes_info
{
	...
	# Data store patterns to call out or ignore. These were identified by Platops.
	# PLEASE VERIFY CORRECT OPERATION IF YOU SET EITHER OF THESE TO THE EMPTY STRING
	# --- infra and management
	my $infra_pattern = "mgmt|infra";
	# --- ignore these data stores (thick provisioned, dedicated, etc...)
	my $exclude_pattern = "esxi|prod-wbnp-app|prod-app-ld5-7";
	...
	foreach my $host (@$host_views) {
		...
		my $store = Vim::get_view(mo_ref => $ref_store, properties => ['summary', 'info']);
		my $value1 = simplify_number(convert_number($store->summary->freeSpace) / 1024 / 1024, 0);
		my $value2 = 0;
                if (convert_number($store->summary->capacity,0) != 0) {
		    $value2 = simplify_number((convert_number($store->summary->capacity) - convert_number($store->info->freeSpace)) / convert_number($store->summary->capacity) * 100);
		}
		my $value3 = simplify_number((convert_number($store->summary->capacity) - convert_number($store->info->freeSpace)) / 1024 / 1024, 0);
		my $s_res = 0;
		if ($perc) {
		    $s_res = $np->check_threshold(check => $value2);
		}
		else {
		    $s_res = $np->check_threshold(check => $value3);
		}
		#$np->add_perfdata(label => $store->summary->name, value => $perc?$value2:$value1, uom => $perc?'%':'MB', threshold => $np->threshold);
		$include_store = 0;
		if (defined($subcommand)) {
		    if (uc($subcommand) eq "ALL")        { $include_store = 1; }
		    elsif (uc($subcommand) eq "INFRA")   { $include_store = ($store->summary->name =~ /$infra_pattern/i)? 1 : 0; }
		    elsif (uc($subcommand) eq "NOINFRA") { $include_store = ($store->summary->name !~ /$infra_pattern|$exclude_pattern/i) ? 1 : 0; }
		    else                                 { $include_store = ($store->summary->name =~ /$subcommand/i) ? 1 : 0; }
		}
		else {
		    $include_store = ($store->summary->name !~ /$exclude_pattern/i) ? 1 : 0; 
		}
		if ( $include_store && !exists($stores_seen{$store->summary->name}) ) {
		    if (($res == 0) || (($res == 1) && ($s_res == 2)) || (($res == 3) && (($s_res == 1) || ($s_res == 2)))) {
			# warning and critical take precedence over unknown
			$res = $s_res;
		    }
		    $stores_seen{$store->summary->name} = [$s_res, $value2 . "% (Used: " . $value3 . " MB, Avail: " . $value1 . " MB)"];
		    my $o_str = $store->summary->name . " ". $value2 . "% (Used: " . $value3 . " MB, Avail: " . $value1 . " MB)";
		    if ($s_res == 0)    { push(@stores_ok, "OK - " . $o_str); }
		    elsif ($s_res == 1) { push(@stores_warning, "WARNING - " . $o_str); }
		    elsif ($s_res == 2) { push(@stores_critical, "CRITICAL - " . $o_str); }
		    else                { push(@stores_unknown, "UNKNOWN - " . $o_str); }
		}
		#$output .= $store->summary->name . "=". $value1 . " MB (" . $value2 . "%), ";
	    }
	}
	$s_total = scalar(@stores_ok) + scalar(@stores_warning) + scalar(@stores_critical) + scalar(@stores_unknown);
	if ($s_total <= 0) {
	    $output = "No data stores were found that match the search criteria";
	}
	else {
	    if ($res == 2) {
		$output .= scalar(@stores_critical) . "/" . $s_total . ", ";
		if (scalar(@stores_warning)) { $output .= scalar(@stores_warning) . " Warn, "; }
		if (scalar(@stores_unknown)) { $output .= scalar(@stores_unknown) . " Unkn, "; }
		$output .= scalar(@stores_ok) . " Ok";
	    }
	    elsif ($res == 1) {
		$output .= scalar(@stores_warning) . "/" . $s_total . ", ";
		if (scalar(@stores_unknown)) { $output .= scalar(@stores_unknown) . " Unkn, "; }
		$output .= scalar(@stores_ok) . " Ok";
	    }
	    elsif ($res == 3) {
		$output .= scalar(@stores_unknown) . "/" . $s_total . ", ";
		$output .= scalar(@stores_ok) . " Ok";
	    }
	    else {
		$output .= scalar(@stores_ok) . "/" . $s_total;
	    }

	    $output .= "\n";

	    foreach my $element (@stores_critical) {
		$output .= $element . "\n";
	    }
	    foreach my $element (@stores_warning) {
		$output .= $element . "\n";
	    }
	    foreach my $element (@stores_unknown) {
		$output .= $element . "\n";
	    }
	    foreach my $element (@stores_ok) {
		$output .= $element . "\n";
	    }
	    chop($output);
	}
