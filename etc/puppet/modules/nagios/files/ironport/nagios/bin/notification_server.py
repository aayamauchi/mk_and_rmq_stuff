...
# path to Jira client
jira_client = os.path.join(os.path.dirname(sys.argv[0]), 'jira-convert')
# Jira projects with "Closed" step
jira_projects_with_closed_step = ["ET"]
# number of concurrent tickets processing threads
concurrent_tickets_processing = 10
...
def send_nexus(i):
    """Create or update Nexus entry."""
    global stats_dict
    global stats_dictlock

    hoststate = {0: 'UP', 1: 'DEGRADED', 2: 'DOWN', 3: 'UNREACHABLE'}
    servstate = {0: 'OK', 1: 'WARNING', 2: 'CRITICAL', 3: 'UNKNOWN'}
    nexusstate = {'OK': 0, 'UP': 0, 'UNKNOWN': 1, 'DEGRADED': 2, 'WARNING': 2,
            'UNREACHABLE': 4, 'MAJOR': 4, 'CRITICAL': 5, 'DOWN': 5}
    ...
    for key in ['alarmcode', 'body', 'subject', 'emsid', 'emstype', 'momid', 'momtype', 'hostaddress']:
        o[key] = i[key]

    try:
        url=i['proto'] + '://' + i['host'] + ':' + i['port'] + i['url']

        # Determine (if needed) AlarmCode, and generate AlertText
        if not i.has_key('servicedesc') or i['servicedesc'] is None:
            # Host issue.
            try:
                i['stateid'] = int(i['hoststateid'])
            except:
                i['stateid'] = int(i['stateid'])
            if i['stateid'] == 0:
                o['alarmcode'] = 'INT-PINGCLR'
            else:
                o['alarmcode'] = 'INT-PINGFAIL'
            o['severity'] = nexusstate[hoststate[i['stateid']]]
            o['link'] = 'https://' + i['emsid'] + '/nagios/cgi-bin/extui.py?host=' + i['hostaddress']
        else:
            # service issue.
            # "severity": "4",  "servicestate": "OK",  "servicestateid": "0",
            i['stateid'] = int(i['servicestateid'])
            if not i['stateid']:
                o['severity'] = 0
            else:
                try:
                    o['severity'] = int(i['severity'])
                except:
                    try:
                        o['severity'] = nexusstate[i['severity'].upper()]
                    except:
                        o['severity'] = nexusstate[servstate[i['stateid']]]
            o['link'] = 'https://' + i['emsid'] + '/nagios/cgi-bin/extui.py?host=' + i['hostaddress']
            o['link'] += '?service=%s' % (i['servicedesc'])

        # Determine AlertClass and set overrides for email alerts.
        log_mesg(o['alarmcode'])
        if o['alarmcode'] in ['INT-PINGCLR', 'INT-PINGFAIL', 'PORT-FAIL', 'PORT-CLEAR', 'INT-SNMPFAIL',
                'INT-SNMPCLR']:
            o['alertclass'] = 'Connectivity'
        elif o['alarmcode'] in ['ISQ-MSG-DELETION', 'ISQ-MSG-QUARANTINE', 'ISQ-MSG-RELEASE',
                'ISQ-MSG-RECEIPENT', 'DNS-BOOTSTRAP-FAIL', 'INVALID-FILTER', 'SENDER-ERROR',
                'CRITICAL-EMAIL-ALERTS', 'Email-Alerts', 'KEY_FINAL_EXPIRING', 'RAID-ALERT',
                'SNMP Trap Alert']:
            o['alertclass'] = 'Security'
            log_mesg("Parsing")
            if o['alarmcode'] == 'CRITICAL-EMAIL-ALERTS' and 'Code:' in i['body']:
                if not i.has_key('stateful') or i['stateful'] in ['', None]:
                    o['stateful'] = False
                try:
                    o['alarmcode'] = i['body'].split('Code:')[-1].split(' Event:')[0]
                except:
                    log_mesg("Error extracting alarm code for email alert.")
                    raise
                try:
                    i['problemid'] = i['body'].split('Event:')[-1].split()[0]
                    i['lastproblemid'] = i['problemid']
                except:
                    log_mesg("Error extracting problem id for email alert.")
                    raise
                try:
                    o['severity'] = nexusstate[i['body'].split('Severity:')[-1].split()[0].upper()]
                except:
                    log_mesg("Error extracting severity for email alert.")
                try:
                    o['stateful'] = int(i['body'].split('Stateful:')[-1].split()[0])
                except:
                    log_mesg("Error extracting state for email alert.")
                    o['stateful'] = False
                try:
                    o['first'] = int(i['body'].split('Start:')[-1].split()[0])
                except:
                    log_mesg("Error extracting initial state change for email alert.")
                try:
                    o['last'] = int(i['body'].split('Last:')[-1].split()[0])
                except:
                    log_mesg("Error extracting last state change for email alert.")
                
                # Munge output and severity.
                if 'Recovery:1' in i['body']:
                    i['body'] = i['body'].replace('CRITICAL', 'OK')
                    o['subject'] = o['subject'].replace('CRITICAL', 'OK')
                    o['severity'] = 0
                body = ''
                for l in i['body'].split('\n'):
                    if not l.startswith('Code:'):
                        body += '%s\n' % (l)
                o['body'] = body
            elif o['alarmcode'] == 'SNMP Trap Alert':
                o['stateful'] = False
                try:
                    o['alarmcode'] = i['body'].split('Code:',1)[-1].split(' Severity:')[0]
                except:
                    log_mesg("Error extracting alarm code for SNMP trap.")
                    raise
                o['alarmid'] = '%s|%s' % (i['hostaddress'], md5('%s %s' % (i['problemid'], o['alarmcode'])).hexdigest())
                try:
                    o['severity'] = nexusstate[i['body'].split('Severity:')[-1].split()[0].upper()]
                except:
                    log_mesg("Error extracting severity for email alert.")
        ...
        if not o.has_key('stateful') or o['stateful'] in ['', None]:
            o['stateful'] = True

        if o['stateful'] and i['stateid'] == 0:
            o['alarmid'] = i['hostaddress'] + '|' + i['lastproblemid']
        elif not o.has_key('alarmid'):
            o['alarmid'] = i['hostaddress'] + '|' + i['problemid']

        statefile = options.prefix + '/var/ns__nexus.state'

        if not o['severity']:
            # is a recovery, set transition to Clear(2) and pull 'first' timestamp from file
            o['transition'] = 2
            # Get state changes in epoch.  store in statefile.  Fix to be more atomic and less
            # potentially impacted by multiple threads.  Could lose state in race condition.
            if i.has_key('up'):
                # host
                try:
                    sf = open(statefile, 'r')
                    ...
                    if state == '':
                        ...
                        state = loads(state)
                    o['first'] = state.get(o['alarmid'], o.get('first', sorted([int(i['down']), int(i['unre'])])[-1]))
                    if o['alarmid'] in state:
                        del state[o['alarmid']]
                    state = dumps(state)
                    sf = open(statefile, 'w')
                    sf.write(state)
                    ...
                    log_mesg('Error removing state from Nexus statefile')
                    log_mesg(traceback.format_exc())
                    o['first'] = sorted([int(i['down']), int(i['unre'])])[-1]
                o['last'] = int(i['up'])
            else:
                # service
                try:
                    ...
                    if state == '':
                        ...
                        state = loads(state)
                    o['first'] = state.get(o['alarmid'], o.get('first', int(i['crit'])))
                    if o['alarmid'] in state:
                        del state[o['alarmid']]
                    state = dumps(state)
                    sf = open(statefile, 'w')
                    ...
                    log_mesg('Error removing state from Nexus statefile')
                    log_mesg(traceback.format_exc())
                    o['first'] = o.get('first', int(i['crit']))
                o['last'] = o.get('last', int(i['ok']))
        else:
            # is an event, set transition to Set(1)
            o['transition'] = 1
            if i.has_key('up'):
                # host
                changes = sorted([int(i['up']), int(i['unre']), int(i['down'])])
                o['first'] = changes[changes.index(int(i['up']))+1]
                try:
                    sf = open(statefile, 'r')
                    state = sf.read()
                    sf.close()
                ...
                try:
                    if state is None or state == '{}':
                        ...
                        state = loads(state)
                    if o['alarmid'] in state:
                        o['first'] = state[o['alarmid']]
                    else:
                        #log_mesg(state)
                        #log_mesg(type(state))
                        #log_mesg(o)
                        #log_mesg(type(o))
                        state[o['alarmid']] = o['first']
                        state = dumps(state)
                        sf = open(statefile, 'w')
                        sf.write(state)
                        sf.close()
                except:
                    log_mesg('Error setting state in Nexus statefile')
                    log_mesg(traceback.format_exc())
                o['last'] = int(time.time())
            else:
                # service
                changes = sorted([int(i['ok']), int(i['unkn']), int(i['warn']), int(i['crit'])])
                o['first'] = changes[changes.index(int(i['ok']))+1]
                if o['stateful']:
                    try:
                        sf = open(statefile, 'r')
                        state = sf.read()
                    ...
                    try:
                        if state is None or state == '{}':
                            ...
                            try:
                                ...
                                # Assume no state if read error.
                                state = {}
                        if o['alarmid'] in state:
                            log_mesg('Munging first from %s to %s from state' % (o['first'], state[o['alarmid']]))
                            o['first'] = state[o['alarmid']]
                        else:
                            state[o['alarmid']] = o['first']
                            state = dumps(state)
        ...
        # Munge device id
        o['deviceid'] = i['deviceid']
        if 'not_set' == o['deviceid']:
            log_mesg('Reconcilation id not_set')
            return
            

        # truncate alarm id
        o['alarmid'] = o['alarmid'][:96]
        #if i['deviceid'].startswith('RE'):
        #    o['deviceid'] = i['deviceid']
        #else:
        #    # use dev default
        #    o['deviceid'] = 'RE221869491800T9JDpgHnOu8AiTlX'

        # truncate body
        o['body'] = o['body'][:255]

        # fix null first for email alerts
        if o['first'] == 0:
            o['first'] = (o['last'] - 120)

        log_mesg('Creating new Nexus ticket for event')
        while attempts < max:
            if not attempts:
                log_mesg('About to execute soap call to \'%s\' with:\n%s' % (url, pprint.pformat(o)))
            else:
                log_mesg('About to re-execute soap call to \'%s\' for %s' % (url, o['alarmid']))
            try:
                Client(url).service.publishAlert(alarmCode=o['alarmcode'], alarmIdentifier=o['alarmid'],
                        alarmComponent='None', alarmCount=1,
                        alertClassification=o['alertclass'], alertText=o['body'],
                        alternateAlertText=o['body'], causeType=0,
                        deviceId=o['deviceid'],
                        emsIdentifier=o['emsid'], emsType=o['emstype'],
                        firstOccurrenceEpochSeconds=o['first'], lastOccurrenceEpochSeconds=o['last'],
                        interfaceAlertEligibility=1, momIdentifier=o['momid'],
                        momType=o['momtype'], node=o['hostaddress'], rootCause=True,
                        severity=o['severity'], statefulAlarm=o['stateful'],
                        transitionType=o['transition'], detailLink=o['link'])
            except:
                attempts += 1
                time.sleep(60)
            else:
        ...
        log_mesg('Ticket creation failed after %i attempts.\n%s' % (attempts, traceback.format_exc()))
...
def send_jira(to, project, subject, body, meta={}):
    ...
    global stats_dictlock
    global jira_client
    global jira_projects_with_closed_step
    subject = subject.replace("'", '"')
    body = body.replace("'", '"')
    # notification attributes
    event_attr = {}
    # regexp to match type, host, service and action
    #event_attr_regexp = re.compile("(\*\*\s+)?([A-Z]+)\s+alert\s+-\s+([a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*\.?)(/[A-Za-z0-9-]+)?\s+is\s+(\w+)(\s+\*\*)?")
    event_attr_regexp = re.compile("(\*\*\s+)?([A-Z]+)\s+alert\s+-\s+([a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*\.?)(/[-A-Za-z0-9\._ ]+)?\s+is\s+(\w+)(\s+\*\*)?")
    event_attr_list = event_attr_regexp.split(subject)
    del event_attr_regexp
    #event_attr['type'] = event_attr_list[2]
    event_attr['host'] = event_attr_list[3]
    event_attr['service'] = event_attr_list[5]
    event_attr['action'] = event_attr_list[6]
    del event_attr_list
    #body = re.sub(r'=*\nTicket Search(.*\n)*?md5', 'md5', body)

    log_mesg('Attempting to open ticket, to: %s, subject: \'%s\'' % (to, subject))
    if not os.path.isfile(jira_client):
        log_mesg('Attempting to open ticket, but jira script missing.  Falling back to email.')
        send_email(to, subject, body)
    else:
        if event_attr['service']:
            cmd = '%s getissues "Summary ~ \'%s%s is\' AND Reporter=nagios' % \
                (jira_client, event_attr['host'], event_attr['service'])
        else:
            # when host is DOWN - search appropriate ticket
            cmd = '%s getissues "Summary ~ \'%s is DOWN\' AND Reporter=nagios' % \
                (jira_client, event_attr['host'])
        ...
        if len(ticket) and project in ticket[-1]:
            ...
            cmd = '%s comment %s \'%s\'' % (jira_client, ticket, body.split('Date/Time', 1)[0])
            ...
            if 'Comment added' not in out and 'RECOVERY' not in subject:
                ...
                if 'RECOVERY' in subject and ('PAGING' not in body or project in jira_projects_with_closed_step):
                    # close the ticket.
                    cmd = '%s cat %s' % (jira_client, ticket)
                    out = subprocess.Popen(shlex.split(cmd), stdout=subprocess.PIPE,
                            stderr=subprocess.PIPE).stdout.read()
                    if 'Assignee: None' in out:
                        # check client
                        if jira_client.find('jira-convert') == -1:
                            # python client
                            # first take the ticket.
                            cmd = '%s take %s' % (jira_client, ticket)
                            out = subprocess.Popen(shlex.split(cmd), stdout=subprocess.PIPE,
                                                   stderr=subprocess.PIPE).stdout.read()
                            # Changes to queue logic has caused some queues to fail a take
                            cmd = '%s update %s assignee nagios' % (jira_client, ticket)
                            out = subprocess.Popen(shlex.split(cmd), stdout=subprocess.PIPE,
                                                   stderr=subprocess.PIPE).stdout.read()
                            if project in jira_projects_with_closed_step:
                                cmd = '%s close %s' % (jira_client, ticket)
                            else:
                                cmd = '%s resolve %s' % (jira_client, ticket)
                            out = subprocess.Popen(shlex.split(cmd), stdout=subprocess.PIPE,
                                                   stderr=subprocess.PIPE).stdout.read()
                        else:
                            # java client wrapper
                            if project in jira_projects_with_closed_step:
                                cmd = '%s takeandclose %s nagios close' % (jira_client, ticket)
                            else:
                                cmd = '%s takeandclose %s nagios resolve' % (jira_client, ticket)
                            out = subprocess.Popen(shlex.split(cmd), stdout=subprocess.PIPE,
                                                   stderr=subprocess.PIPE).stdout.read()
                        if out != '':
            ...
            if project == 'ET':
                if product == 'scansafe':
                    if 'is CRITICAL' in subject or 'is DOWN' in subject:
                        priority = 1
                    else:
                        priority = 2
                else:
                ...
                cmd = '%s create --project=%s --summary=\'%s\' --description=\'%s\' ' % \
                        (jira_client, project, subject, body)
                cmd += '--type=Event --field=%s:"%s" --field=%s:"%s" ' % \
                        (product_id, product, environment_id, environment)
...
def send_email(to, subject, body):
    ...
    mesg['From'] = 'Nagios Pseudo-user <nagios@mon.ops.ironport.com>'
    try:
...
def parse_if(command, section, sectiontext, i, item, cache):
    ...
    if command[0] == 'script':
        ...
        _i, cache = parse_script(cmd, item, '', cache)
        if options.loglevel == log_debug:
...
class ReadSpoolThread(threading.Thread):
    ...
    def run(self):
        ...
        global spoolqueue_dict
        global spoolqueue_dictlock
        global concurrent_tickets_processing
        spool_dir = '%s/notification_spool/' % (options.prefix)
        while 1:
            spools = os.listdir(spool_dir)
            for spoolfile in spools[:concurrent_tickets_processing]:
    ...
    while 1:
        ...
        for spool_tuple in spool_tuples:
            ...
            if spool_tuple in spoolqueue_dict and len(spoolqueue_dict[spool_tuple]) and \
                ...
                release_lock(spoolqueue_dictitemlock[spool_tuple], \
                        'spoolqueue_dictitemlock[%s]' % (str(spool_tuple)))
            if os.getloadavg()[0] > 12.0:
                log_mesg("load average high, throttling threadspawn.")
                time.sleep(0.5)
            if os.getloadavg()[0] > 20.0:
                time.sleep(2)
