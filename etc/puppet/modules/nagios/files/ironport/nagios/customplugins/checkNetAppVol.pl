#!/usr/local/bin/perl --
# 
# This script checks for disk space and inodes via SNMP on the NetApp.
...
# Modified By:
# Mike Lindsey <miklinds@ironport.com>
#
# Usage:
...
for($i=1;$i < 90; $i++) {
	$oid = ".1.3.6.1.4.1.789.1.5.4.1.10.$i";
...
$oid = ".1.3.6.1.4.1.789.1.5.4.1.6.$index";
open(GET,"/usr/bin/snmpget -r 2 -t 15 -v1 -c $community $host $oid 2>&1|");
while(<GET>) {
	next unless /INTEGER:/;
	chomp;
...
# get the percent inodes of the mountpoint we are searching for
...
if($percentfull > $critical || $percentinodes > $critical) {
	print "CRITICAL - $mountpoint is at $percentfull% of disk capacity [inodes $percentinodes%]| disk_pct=$percentfull;$warn;$critical;;\n";
	exit 2;
}
if($percentfull > $warn || $percentinodes > $warn) {
	print "WARNING - $mountpoint is at $percentfull% of disk capacity [inodes $percentinodes%]| disk_pct=$percentfull;$warn;$critical;;\n";
	exit 1;
}

print "OK - $mountpoint is at $percentfull% of disk capacity [inodes $percentinodes%]| disk_pct=$percentfull;$warn;$critical;;\n";
exit 0;
...
sub print_usage () {
        print "Usage: 
        command_line    \$USER1\$/checkNetAppDiskSpace.pl -H \$HOSTADDRESS\$ -C \$ARG1\$ -m \$ARG2\$ -w \$ARG3\$ -c \$ARG4\$
 Where:
       \$USER1\$ = path to Nagios(R) plugin directory
				\$HOSTADDRESS\$ = IP of the netapp to be tested
				\$ARG1\$ = SNMP Read string
