#!/bin/env perl
############################## check_snmp_load #################
# SNMP check for Cisco ASA firewalls / ASR series routers
my $Version='1.0';
# Date    : 2009/11/23
# Author  : Mathias Mahnke
# Licence : GPL - http://www.fsf.org/licenses/gpl.txt
# Contributors:
#  Patrick Proy (check_snmp_cpu)
#  Roberto.Fuentes@nconsulting.es 2001/06/30 added ASA 5520 CPULoad & Memory oids 
#
# vmashkov@cisco.com 2013/01/04 Added OID for ASA/ASR on Khalifa hosts
#
#################################################################
#
# Help : ./check_snmp_asa.pl -h
#
...
# Cisco ASA
# Cisco ASA IPS CPU
my $cisco_asa_cpu_5m = "1.3.6.1.4.1.9.9.109.1.1.1.1.5.1"; # Cisco CPU load (5min %)
my $cisco_asa_cpu_1m = "1.3.6.1.4.1.9.9.109.1.1.1.1.4.1"; # Cisco CPU load (1min %)
my $cisco_asa_cpu_5s = "1.3.6.1.4.1.9.9.109.1.1.1.1.3.1"; # Cisco CPU load (5sec %)
# Cisco ASA IPS Memory
...
# Cisco ASR 
# Cisco ASR IPS CPU
my $cisco_asr_cpu_5m = ".1.3.6.1.4.1.9.9.109.1.1.1.1.8.2"; # Cisco CPU load (5min %)
my $cisco_asr_cpu_1m = ".1.3.6.1.4.1.9.9.109.1.1.1.1.7.2"; # Cisco CPU load (1min %)
my $cisco_asr_cpu_5s = ".1.3.6.1.4.1.9.9.109.1.1.1.1.7.2"; # Cisco CPU load (5sec %)
#################################################################

# valid values
my @valid_types = ("cpu","mem","health");
my @valid_devices = ("asa", "asr");
...
my $o_check_type= "cpu";        # check type  : cpu | mem
my $o_device =  undef;          # Cisco device; asa or asr
# End compatibility
...
-d, --device=DEVICE
   Mandatory value, can be "asa" or "asr: only
...
my $D_option_valid=0;
foreach (@valid_devices) { if ($_ eq $o_device) {$D_option_valid=1} };
if ( $D_option_valid == 0 )
        {print "Invalid device type (-d option, you set $o_device)!\n"; print_usage(); exit $ERRORS{"UNKNOWN"}}
...
# set IODs depending on ASA/ASR
my $cisco_cpu_5m = undef;
my $cisco_cpu_1m = undef;
my $cisco_cpu_5s = undef;
if ($o_device = "asa") {
    $cisco_cpu_5m=$cisco_asa_cpu_5m;
    $cisco_cpu_1m=$cisco_asa_cpu_1m;
    $cisco_cpu_5s=$cisco_asa_cpu_5s;
} elsif ($o_device = "asr") {
    $cisco_cpu_5m=$cisco_asr_cpu_5m;
    $cisco_cpu_1m=$cisco_asr_cpu_1m;
    $cisco_cpu_5s=$cisco_asr_cpu_5s;
} else  {
    print "Invalid device type (-d option), you set $o_device!\n"; print_usage(); exit $ERRORS{"UNKNOWN"}
}
...
print "Cisco ASA/ASR CPU : 5sec = $load[0] %, 2min = $load[1] %, 5min = $load[2] % :";
...
print "Cisco ASA/ASR Memory : used = $load[0] MB, free = $load[1] MB, utilization = $load[2] % :";
