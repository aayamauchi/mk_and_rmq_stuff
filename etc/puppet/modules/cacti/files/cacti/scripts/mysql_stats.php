...
if ($_SERVER["argc"] == 5 || ($_SERVER["argv"][1] == "status" && $_SERVER["argc"] == 6)) {
    ...
    if (@mysql_connect($host, $username, $password)) {
        $serverStatus = @mysql_query("select @@version");
        ...
        if ($mysqlMajor < 5) {
            ...
            $result_stat = @mysql_query("SHOW /*!50002 GLOBAL
*/ STATUS");
        } else {
        ...
        if ($_SERVER["argv"][1]=='innodb') {
            $result_var = @mysql_query("show innodb status");  
         ...
         */
        $result_var = @mysql_query("show slave status");
