...
WALKOUT=`snmpwalk -v 1 -Os -c ${community} ${hostname} .1.3.6.1.2.1.4.20.1.2`
walkstatus=$?

echo ${WALKOUT} | grep INTEGER >> /dev/null 2>&1

integerstatus=`echo $?`

if [ "$integerstatus" -ne 0 ]; then
    echo "UNKNOWN - SNMP not responding on ${hostname} when requesting VIP IP ${IP}"
    echo snmpwalk exit code: ${walkstatus}
    exit 3
fi

echo ${WALKOUT} | grep ${IP} >> /dev/null 2>&1
