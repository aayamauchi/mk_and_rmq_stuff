...
use lib "/usr/local/lib64/perl5/site_perl/5.8.8/";
...
my $nets_ram_buffer     = "1.3.6.1.4.1.2021.4.14.0"; # Real memory buffers
...
if (defined ($o_netsnmp)) {
  ...
  :
    ($$resultat{$nets_ram_total}-($$resultat{$nets_ram_free}+$$resultat{$nets_ram_cache}+$$resultat{$nets_ram_buffer}))/$$resultat{$nets_ram_total};
