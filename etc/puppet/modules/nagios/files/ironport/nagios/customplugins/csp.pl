...
my $startdate = `/bin/date`;
$startdate = chomp($startdate);
...
$SIG{'ALRM'} = sub {
     my $enddate = `/bin/date`;
     $enddate = chomp($enddate);
     print ("ERROR: Alarm signal (Nagios time-out) [TIMEOUT: $TIMEOUT] [o_timeout: $o_timeout] $startdate :: $enddate\n");
     exit $ERRORS{"UNKNOWN"};
};
...
if ( !defined ($o_path) ) {
  verb("!o_path");
		$resultat = $session->get_table($run_name_table);
