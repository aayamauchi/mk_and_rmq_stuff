...
$pw_location = '/etc/cacti/cactiwmi.pw/'; // location of the password files, ensure the trailing slash
$log_location = '/data/cacti/log/'; // location for the log files, ensure trailing slash
...
if ($wmi_count > 2) { // Check for transient errors and shit them off
        while (strpos($wmiout[0], "failed NT status")) {
                array_shift($wmiout);
                $wmi_count -= 1;
        };
};
