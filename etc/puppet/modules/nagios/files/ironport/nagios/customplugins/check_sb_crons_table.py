...
:Status: $Id: //sysops/main/puppet/test/modules/nagios/files/ironport/nagios/customplugins/check_sb_crons_table.py#7 $
:Authors: aflury
"""
...
USAGE = """
%s -H <host> -u <user> -p <password> -d <db> [-c <critical> -w <warning> | -t] [-C <command>] [-L local_host]
...
local_host - Local crons host name (implies -t)
""" % (sys.argv[0])
...
if __name__ == '__main__':
    ...
    if opt.local_host and not opt.host:
        print USAGE
        sys.exit(2)
    ...
    if opt.local_host:
        if len(rows) != 1:
            print 'CRITICAL - Expected a single row in crons for %s, but got back %d' % (opt.command, len(rows))
            sys.exit(2)

        opt.critical = int(rows[0]['exectime_error'])
        opt.warning  = int(rows[0]['exectime_warning'])

        try:
            cursor.execute(query_local0)
        except:
            print 'CRITICAL - Error querying for %s in crons_local' % (opt.command)
            sys.exit(2)
        else:
            rows = cursor.fetchall()

    for row in rows:
        if opt.thresholds and not opt.local_host:
            try:
        ...
        elif runlength and runlength > opt.warning:
            row['msg'] = 'runtime exceeds threshold'
            warnings.append(row)
        elif (row['lastsuccess'] and row['lastrunstart'] < row['lastsuccess'] 
		and (now - row['lastsuccess']) > (frequency + opt.critical)):
            row['msg'] = 'last success was %s' % (fmt_time(row['lastsuccess']))
            criticals.append(row)
        elif (row['lastsuccess'] and row['lastrunstart'] < row['lastsuccess']
		and (now - row['lastsuccess']) > (frequency + opt.warning)):
            row['msg'] = 'last success was %s' % (fmt_time(row['lastsuccess']))
            warnings.append(row)
    ...
    if opt.command == None:
        ...
        try:
            ...
            if opt.local_host:
                print "UNKNOWN - %s/%s not found in crons_local table" % (opt.command, opt.local_host)
            else:
                print "UNKNOWN - %s not found in crons table" % (opt.command)
            sys.exit(3)
