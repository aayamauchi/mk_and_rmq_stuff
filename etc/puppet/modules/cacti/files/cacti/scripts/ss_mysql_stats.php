 ...
 *
 * Converted to Script Server by TheWitness
 * larryjadams at comcast do net - 2006/10/05
 *
 * usage:
 * from the command line:
 * ss_mysql_stats.php <section> db_host db_user db_password [status_var]
 *
 * as a script server object:
 * ss_mysql_stats.php ss_mysql_stats <section> db_host db_user db_passwd [status_var]
 *
 * Usage: mysql_stats.php <section> <db_host> <db_user> <db_password> [<status_var>]
 * Where <section> is one of: cache, command, handler, thread, traffic,
 *                            innodb, tevolatilequery, tevolatilehandler, teselectquery,
 *                            teselecthandler, teindexusage, tethreadsabends, tereplication,
 *                            tenetwork, tequerycache, tetempobjects, tesorts, telockingandslow,
 *                            tediskactivity, teadmincommand, teserverstatus
 *
*/
...
if (!isset($called_by_script_server)) {
	...
	print call_user_func_array("ss_mysql_stats", $_SERVER["argv"]);
}

function ss_mysql_stats($section = "", $host = "", $user = "", $passwd = "", $status_var = "") {
	...
	if ($connection) {
		$serverStatus = @mysql_query("select @@version", $connection);
		$statLine = @mysql_fetch_row($serverStatus);
		...
		if ($mysqlMajor < 5) {
			$result_stat = @mysql_query("show status", $connection);
		}else{
			$result_stat = @mysql_query("show global status", $connection);
		}
		...
		$result_var = @mysql_query("show variables", $connection);
		while ($fld_var = @mysql_fetch_row($result_var)) {
			$variables[$fld_var[0]] = $fld_var[1];
		}

		if ($section == 'innodb') {
			$result_var      = @mysql_query("show innodb status", $connection);
			$rawinnodbstatus = @mysql_fetch_assoc($result_var);
		 ...
		 */
		$result_var  = @mysql_query("show slave status", $connection);
		$slavestatus = @mysql_fetch_assoc($result_var);
	}else{
		cacti_log("ERROR: MySQL Stats - Connect failed. Check MySQL parameters (host/login/password)");
		return "U";
	}

	if (!is_array($status) || !is_array($variables)) {
		cacti_log("ERROR: MySQL Stats - Cannot get statistics. Check MySQL server permissions");
	...
	case "status" :
		if (empty($status_var)) {
			cacti_log("ERROR: MySQL Stats - Wrong parameter count.");
			@mysql_close($connection);
			return "U";
		}

		$output = $status[$status_var];

		break;
	default :
		cacti_log("ERROR: MySQL Stats - Unknown section request");
		@mysql_close($connection);
		return "U";
	}
