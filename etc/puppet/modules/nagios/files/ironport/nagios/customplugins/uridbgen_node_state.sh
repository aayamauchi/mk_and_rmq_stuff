#!/usr/local/bin/bash
#==============================================================================
# uridbgen_node_state.sh
#
# Performs two different checks:
#    When called with -c, checks if the number of uridbgen connected nodes
#    equals the number we have configured in ASDB and that they all
#    match by name.
#
...
ASDB_CACHE="/tmp/cache-uridbgen-nodes"
...
if [ ${CHECK_ACTIVE} -eq 1 ]; then
    ...
    if [ ${N_CONFIGURED_NODES} -ne ${N_NODES} ]; then
        echo "CRITICAL - Number of connected nodes (${N_NODES}) not equal number of configured nodes (${N_CONFIGURED_NODES})"
        exit 2
    fi
fi
