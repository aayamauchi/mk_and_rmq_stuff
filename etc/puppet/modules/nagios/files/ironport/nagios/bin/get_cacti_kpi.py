...
def init():
    ...
    parser.add_option("-a", "--asdb", type="string", dest="asdb",
    ...
    parser.add_option("--share", action="store_true", dest="share",
                            help="Print Netapp Share utilization stats.  Ignores --data, --source, -N; assumes --perhost.  If -n is specified, it is viewed as a CSV of hosts to query.  Output is last for time slice.")
    ...
    exit = 0
    special = 0
    if options.cpu: special += 1
    if options.memory: special += 1
    if options.disk: special += 1
    if options.swap: special += 1
    if options.share: special += 1
    if special > 1:
        print "--cpu, --memory, --swap, --disk, and --swap are mututally exclusive."
        exit = 1
    elif options.cpu or options.memory or options.disk or options.swap or options.share:
...
def init_db():
    if options.verbose: sys.stderr.write(">>DEBUG start  - " + funcname() + 
                            "()\n")
    conn = MySQLdb.connect (host = "cacti-db1.soma.ironport.com",
                            user = "dashboard",
                            passwd = "asdf1234",
                            db = "cacti")
    if options.verbose: sys.stderr.write(">>DEBUG end    - " + funcname() + 
    ...
    for host in value_dict:
        for file in value_dict[host]:
            ...
            if options.listfiles:
                print file
    if len(total_dict['values']) == 0 and not (options.memory or options.share):
...
if options.cpu:
    ...
    if options.perhost:
        ...
        if len(total_dict['total']['values']):
            print "Memory%% %.2f" % (100-((total_dict['free']['total']/
                    total_dict['total']['total']) * 100))
        else:
            print "Memory%% %.2f" % (100-((total_dict['fBSDmemcache']['total']+
                    total_dict['fBSDmemfree']['total']+
                    total_dict['fBSDmeminactive']['total'])/
                    (total_dict['fBSDmemtotal']['total']))*100)
elif options.disk:
    options.quiet = True
    ...
    for host in hostlist:
        ...
        value_dict[host] = {}
        mount_dict[host] = {}
        sql = "SELECT name_cache FROM data_template_data WHERE name_cache LIKE "
        sql += "'%s - Partition - %%'" % (host)
        mounts = do_sql(sql)
        for mount in mounts:
            mount = mount[0].split()[-1]
            sql = "SELECT field_value FROM host_snmp_cache WHERE host_id = (SELECT id FROM "
            sql += "host WHERE hostname = '%s') AND field_name LIKE 'dskPath' AND snmp_index "
            sql += "= (SELECT snmp_index FROM host_snmp_cache WHERE field_value = '%s' AND "
            sql += "field_name = 'dskDevice' AND snmp_query_id = 2 AND host_id = (SELECT id "
            sql += "FROM host WHERE hostname = '%s') LIMIT 1)" 
            sql = sql % (host, mount, host)
            try:
                path = do_sql(sql)[0][0]
            except:
                path = mount
    ...
    if options.perhost:
        ...
        print "Swap%% %.2f" % (100-(total_dict['Available']['total']/
                total_dict['Total']['total'])*100)
elif options.share:
    total_dict = {}
    value_dict = {}
    if options.end is 0:
        options.start = -1800
    options.data = 'NetApp - Storage Volume Usage'
    print "filer:/share,used%,freeKB,usedKB,totalKB"
    if not options.cache:
        hostlist = []
        for host in do_sql("SELECT hostname FROM host WHERE host_template_id=(SELECT id FROM host_template WHERE name='%s')" % (options.host)):
            host = host[0]
            hostlist.append(host)
    else:
        hostlist = options.cache.split(',')
    for host in hostlist:
        total_dict[host] = {}
        value_dict[host] = {}
        sql = "SELECT name_cache FROM data_template_data WHERE name_cache LIKE "
        sql += "'%s - Volume Usage - %%' AND name_cache NOT LIKE '%%/.sn%%'" % (host)
        mounts = do_sql(sql)
        for mount in mounts:
            mount = mount[0].split()[-1]
            options.cache = "%s %% %s" % (host, mount)
            total_dict[host][mount] = {}
            value_dict[host][mount] = {}
            for source in ['Avail', 'Total', 'Used']:
                options.source = 'KBytes%s' % (source)
                total_dict[host][mount][source] = {}
                value_dict[host][mount][source] = {}
                total_dict[host][mount][source], value_dict[host][mount][source] = get_data()

    for host in value_dict:
        for mount in value_dict[host]:
            try:
                file = value_dict[host][mount]['Avail']['x'].keys()[0]
                free = value_dict[host][mount]['Avail']['x'][file]['last']
                used = value_dict[host][mount]['Used']['x'][file]['last']
                total = value_dict[host][mount]['Total']['x'][file]['last']
                usedpct = (used/total) * 100
                print "%s:%s,%.2f,%s,%s,%s" % (host, mount, usedpct, free, used, total)
            except:
                pass
else:
