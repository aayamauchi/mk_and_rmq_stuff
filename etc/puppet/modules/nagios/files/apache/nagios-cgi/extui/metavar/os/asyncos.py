...
def connect(hostname):
    ...
    global error_str
    username = 'hosted_metrics'
    password = '4e8dcaf44675638b94edb71b55cff577'
    error = 0
    try:
...
def get_data(hostname):
    ...
    (client, channel, transport) = connect(hostname)
...
def main(host, service, args, host_s, service_s):
    ...
    audit = {'tabs': [{'header': 'Status Detail',
                'body': '<iframe src=/nagios/audit/%s width=98%% height=1000><p>No iframe support</p></iframe>' % 
                            (host + '.html')}]}
    if not os.path.exists(file) or os.stat(file).st_mtime < (time.time() - 600):
        try:
        ...
        try:
            fh = open(file, 'w')
            fh.write('<html><head><title>%s</title></head><body>\n<pre>\n' % (host))
            fh.write(out)
            fh.write('\n</body></html>')
            fh.close()
        except:
