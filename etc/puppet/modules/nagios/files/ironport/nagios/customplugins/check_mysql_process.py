...
Other than the infofilter, filters can be inverted by prepending !"""
    ...
    parser.add_option("-E", "--explain", action="store_true", dest="doexplain",
                            default=False,
                            help="do sql explain on matching threads (requires -P)")
    parser.add_option("-k", "--kill", action="store_true", dest="killthread",
                            default=False,
                            help="Kill matching Critical threads")
    parser.add_option("-K", "--killdupes", action="store_true", dest="killdupes",
                            default=False,
                            help="""Kill matching Critical threads, leaving youngest
                            matching thread, alone.""")
...
for val in do_sql("mysql", "SHOW FULL PROCESSLIST"):
    ...
    if options.warning and val[5] < options.warning:
        continue
    elif options.warning and val[5] >= options.warning:
        warning = 1
	x['warning'] = 1
...
if critical:
    print "CRITICAL threads matching thresholds: %s [##p3##]" % (critpids.replace(',', ', '))
    if len(warnpids): print "WARN threshold for: %s" % (warnpids.replace(',', ', '))
    print
elif warning:
    print "WARNING threads matching thresholds: %s [##p3##]" % (warnpids.replace(',', ', '))
else:
    if len(pids):
        print "CRITICAL threads matching filters: %s [##p3##]" % (pids.replace(',', ', '))
	critical = 1
    else:
...
if options.killdupes:
    seen = {}
    for process in critpids.split(','):
        process = long(process)
        if process in processlist.keys() and \
                processlist[process].has_key('info') and \
                seen.has_key(processlist[process]['info']):
            try:
                do_sql("mysql", "KILL %s" % (process))
            except:
                print "Unable to kill %s" % (process)
            else:
                print "Killed %s" % (process)
        elif process in processlist.keys() and \
                processlist[process].has_key('info'):
            seen[processlist[process]['info']] = 1
        else:
            print process, long(process) in processlist.keys()
elif options.killthread and (critical or warning):
    for process in critpids.split(','):
        try:
            do_sql("mysql", "KILL %s" % (process))
        except: 
