#!/usr/bin/python26 -u
# -*- coding: ascii -*-
"""Script for submitting nagios events or notification data to ASDB.
...
def init():
    ...
    parser.add_option("-u", "--url", type="string", dest="url",
                            default="http://asdb.ironport.com/nagios/",
                            help="Base Url.  Default='http://asdb.ironport.com/nagios/'")
    ...
    parser.add_option("-j", "--jira", type="string", dest="jira",
                            default="/usr/local/ironport/nagios/bin/jira",
    ...
    parser.add_option("-m", "--macros", type="string", dest="macros",
                            ...
                            metavar="'<MACRO>:<VALUE>|...'")
    parser.add_option("--noenv", action="store_true", dest="noenv",
    ...
    if options.macros is not None:
	    ...
	    if name.upper() not in ['HOSTNAME', 'SERVICEDESC', 'HOSTSTATE', 'HOSTSTATETYPE',
                    'SERVICESTATE', 'SERVICESTATETYPE', 'HOSTOUTPUT', 
                    'SERVICEOUTPUT', 'LONGHOSTOUTPUT', 'LONGSERVICEOUTPUT']:
                continue
            if name.upper() == 'HOSTNAME': name = 'HOST'
            if name.upper() == 'SERVICEDESC': name = 'SERVICE'
            env[name.lower()] = value.replace('\\n', '\n')
            if options.verbose:
                print "Adding %s('%s') to env dict." % (name, value)
    if 'host' not in env:
...
def get_ticket(host, service=None):
    """Wait up to 120 seconds, attempting to get ticket."""
    ticket = None
    start = time.time()
    ...
    while now < start + 120:
        ...
        ticket = subprocess.Popen(shlex.split(cmd), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        serr = ticket.stderr.readlines()
        ticket = ticket.stdout.readlines()
        if options.verbose:
            for t in ticket:
                print t
            print "err:"
        ...
        if len(ticket) and ('ticket' in ticket[-1] or subject in ticket[-1]):
...
if __name__ == '__main__':
    ...
    if options.notification:
        ...
        if options.rt:
            ticketurl = 'https://it-tickets.ironport.com/Ticket/Display.html?id='
        elif options.jira:
            ticketurl = 'https://jira.sco.cisco.com/browse/'
        try:
