...
# $Id: //sysops/main/puppet/test/modules/nagios/files/ironport/nagios/customplugins/ldap-pwgen.py#1 $
...
if __name__ == '__main__':
    ...
    try:
        ...
        config_file = "/usr/local/ironport/sysops/centralizedauth/centralizedauth.cfg"
    ...
    system_user_location = config.get('DEFAULT', 'system_user_location')
    results = ldap_session.search_s(system_user_location, ldap.SCOPE_SUBTREE,
    ...
    if not results == None:
        for dn, values in results:
            login = values['uid'][0]
            if login not in system_map: 
                system_map[login] = True
                system_users.append(values)
    else:
        sys.stderr.write("Error: No system users found.  Aborting.\n")
        sys.exit(1)

    system_users.sort(sort_uids)
    users = system_users
	...
	# re-arrange as needed in .sh for other platforms
        print "%s:%s:%s:%s::0:0:%s:%s:%s" % (login, password, uid, gid,
            gecos, homedir, shell)
