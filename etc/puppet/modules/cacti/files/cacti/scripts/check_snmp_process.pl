...
for (my $i=0; $i< $num_int; $i++) {
   ...
   if (!isnotnum($state)) { # check argument is numeric (can be NoSuchInstance)
     $num_int_ok++ if (($state == 1) || ($state ==2) || ($state ==4));
   }
}
...
if (defined ($o_cpu) ) {
  ...
  if ($return ==0) {
    ...
    do {
	  ...
	  if ($found_value <0) { # in case of program restart
		...
		$j=0;$found_value=0; # don't look for more values
		$n_rows=0; # reset file
	  }
        }
      }
  ...
  if (defined($found_value)) {
    if ($found_value > $o_cpuL[1]) {
      $final_status=2;
      $cpu_print.=", Cpu : ".sprintf("%.0f",$found_value)."% gt ".$o_cpuL[1]." CRITICAL";
    } elsif ( $found_value > $o_cpuL[0]) {
      $final_status=($final_status==2)?2:1;
      $cpu_print.=", Cpu : ".sprintf("%.0f",$found_value)."% gt ".$o_cpuL[0]." WARNING";
    } else {
