...
$Id: //sysops/main/puppet/test/modules/nagios/files/ironport/nagios/customplugins/feeds_getter_check.py#1 $
...
#import common.bin.base_nagios_plugin as nagios_plugins
...
def build_result_msg(verbosity, result, rule_data):
    ...
    for level in (nagios_plugins.RESULT_CRITICAL, nagios_plugins.RESULT_WARNING):
        ...
        if verbosity < 2:
            ...
            feedstr = ''.join(lines)
            feedstr = feedstr.split("'")[1::2]
            msg += '%d Feeds Beyond Max Age: %s\n' % (len(lines), feedstr) + '\n'.join(lines)
        else:
