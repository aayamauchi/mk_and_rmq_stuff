...
$Id: //sysops/main/puppet/test/modules/nagios/files/ironport/nagios/customplugins/wbnp_importer_check-espetus.py#1 $
...
def build_result_msg(result, verbosity, critical_threshold, warn_threshold,
    ...
    if state is None:
        ...
        time_delta = now_time-last_logts

        suffix = ''
        if verbosity > 0:
            suffix += ' (past threshold = %s)' % (threshold,)
        if verbosity == 2:
            suffix += '\nThe last processed log timestamp is %s ' % (last_logts, )
...
def check_wbnp_importer_status(opt, args):
    ...
    count_sql = """
                SELECT COUNT(*)
                FROM high_water_marks hi, data_apps da
                WHERE UNIX_TIMESTAMP(hi.ctime) > (UNIX_TIMESTAMP(now()) - %%d)
                  AND hi.data_app_id = da.data_app_id
                  AND da.app_name = '%s'
                  AND host_name = '%s';
                """ % (opt.app_name, opt.app_server)
    ...
    last_logts_sql = """
                SELECT MAX(ctime)
                FROM high_water_marks hi, data_apps da
                WHERE hi.data_app_id = da.data_app_id
                  AND da.app_name = '%s'
                  AND host_name = '%s';
                """ % (opt.app_name, opt.app_server)
    ...
    try:
        ...
        nagios_plugins.check_db_schema(conn, 'high_water_marks', ('data_app_id', 'ctime',))
